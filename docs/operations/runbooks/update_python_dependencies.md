# HOWTO Update Python version and Dependencies
...
If you're updating the Python version, follow these steps:
...
3. Update the [security checks](../../../.github/actions/django-security-check/Dockerfile) Python version to the correct version. This is needed for the security checks that run in CI/CD.

4. Run `docker compose down` to spin down the current containers and images so we can start afresh.

5. Run `docker compose build` to rebuild the container with a new image with the new Python version. Note this will still be using the old Pipfile.lock, which means if there are completely incompatible dependencies you will start to see errors or warnings. Watch for these errors but don't troubleshoot yet, you're not done.

6. Now that we have a new container, we can update all the requirements. This may automatically remove errors and warnings you saw in the last step. Run:
...
7. Run `docker compose build` a second time to pick up the freshly generated lock file.

8. Troubleshoot any errors or warnings that didn't go away on the build or which pop up when testing the app.
