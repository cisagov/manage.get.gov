...
class EPPConnectionPool:
    ...
    def stats(self):
        ...
        return {
            "size": self.size,
            "connections created": self._connections_created,
            "idle": self._idle.qsize(),
            "in use": self._connections_created - self._idle.qsize(),
        }
    ...
    def _maintenance_loop(self):
        ...
        while self.heartbeat_interval > 0:
            ...
            try:
                ...
                logger.warning(
                    "EPP pool heartbeat pass failed. Once is fine, but investigate "
                    "if you see multiple back to back. Stats: %s",
                    self.stats(),
                    exc_info=True,
                )
    ...
    def _replenish(self):
        ...
        while self._connections_created < self.size and self._can_create():
            try:
                ...
                self._release_slot()
                logger.error(
                    "Replenish hit an error & failed to build a connection. Stats: %s", self.stats(), exc_info=True
                )
