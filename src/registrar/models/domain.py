...
class Domain(TimeStampedModel, DomainHelper):
    ...
    def _set_singleton_contact(self, contact: PublicContact, expectedType: str):
        ...
        domainContactExists = PublicContact.objects.filter(
            registry_id=contact.registry_id
        ).exists()
        contactIsAlreadyOnDomain = PublicContact.objects.filter(
            domain=self,
            registry_id=contact.registry_id,
            contact_type=contact.contact_type,
        ).exists()
        contactOfTypeExists = PublicContact.objects.filter(
    ...
    def security_contact(self, contact: PublicContact):
        ...
        if contact.email is None or contact.email == "":
            contact.email = "dotgov@cisa.dhs.gov"
    ...
    def _make_contact_in_registry(self, contact: PublicContact):
        ...
        # Do not disclose if we are using the default email
        create.disclose = self._disclose_fields(
            isSecurity and contact.email != "dotgov@cisa.dhs.gov"
        )
        try:
    ...
    def _fetch_cache(self, fetch_hosts=False, fetch_contacts=False):
        ...
        try:
            ...
            logger.info(f"_fetch_cache()->  uncleaned {cache}")
