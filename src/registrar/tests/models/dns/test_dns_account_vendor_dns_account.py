...
class DnsAccount_VendorDnsAccountTest(TestCase):
    ...
    def setUp(self):
        ...
        self.join1 = AccountsJoin.objects.create(
    ...
    def test_is_active_constraint_passes(self):
        ...
        second_join = AccountsJoin.objects.create(
            dns_account=self.dns_account, vendor_dns_account=vendor_account_2, is_active=False
        )

        self.assertTrue(AccountsJoin.objects.filter(pk=second_join.pk).exists(), "Second join created successfully!")
        self.assertTrue(
            AccountsJoin.objects.filter(dns_account=self.dns_account, is_active=True).count() == 1,
    ...
    def test_update_is_active_constraint_throws_error(self):
        ...
        join2 = AccountsJoin.objects.create(
            dns_account=self.dns_account,
            vendor_dns_account=vendor_account_2,
            is_active=False,
        )
