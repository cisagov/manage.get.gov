...
class TestDnsHttpClient(SimpleTestCase):
    ...
    @override_settings(DNS_MOCK_EXTERNAL_APIS=True, IS_PRODUCTION=False)
    @mock.patch("registrar.services.mock_cloudflare_service.MockCloudflareService.start")
    def test_build_dns_client_starts_mock_only_when_needed(self, mock_start):
    ...
    @override_settings(DNS_MOCK_EXTERNAL_APIS=True, IS_PRODUCTION=True)
    @mock.patch("registrar.services.mock_cloudflare_service.MockCloudflareService.start")
    def test_build_dns_client_does_not_start_mock_in_production(self, mock_start):
        client = build_dns_client()
        try:
