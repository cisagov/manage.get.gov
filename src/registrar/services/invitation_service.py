...
def invite_to_portfolio(
    ...
    if check_duplicate_portfolio_invitation(email, portfolio):
        raise InvitationError(
            f"{email} has already been invited to this portfolio "
    ...
    try:
        with transaction.atomic():
            ...
            logger.info(
                f"Created portfolio invitation for {email} "
                f"to portfolio {portfolio.id}"
            )
            return permission
...
def invite_to_domain(
    ...
    try:
        with transaction.atomic():
            ...
            logger.info(
                f"Created domain invitation for {email} "
                f"to domain {domain.id}"
            )
            return domain_role
...
def invite_to_domains_bulk(
    ...
    try:
        with transaction.atomic():
            ...
            existing_roles = {
                role.domain_id: role
                for role in UserDomainRole.objects.filter(
                    email=email, domain_id__in=domain_ids
                )
            }
            ...
            logger.info(
                f"Created bulk domain invitations for {email} "
                f"to {len(domain_list)} domains"
            )
            return domain_roles
...
def get_pending_invitations(user: User):
    ...
    domain_roles = UserDomainRole.objects.filter(
        email=email, status=UserDomainRole.Status.INVITED
...
def accept_portfolio_invitation(user: User, portfolio: Portfolio):
    ...
    try:
        with transaction.atomic():
            ...
            logger.info(
                f"User {user.id} accepted portfolio invitation "
                f"for {portfolio.id}"
            )
            return permission
...
def cancel_domain_invitation(email: str, domain: Domain):
    ...
    try:
        with transaction.atomic():
            ...
            if canceled:
                logger.info(
                    f"Canceled domain invitation for {email} "
                    f"to domain {domain.id}"
                )
...
def cancel_portfolio_invitation(email: str, portfolio: Portfolio):
    ...
    try:
        with transaction.atomic():
            ...
            if canceled:
                logger.info(
                    f"Canceled portfolio invitation for {email} "
                    f"to portfolio {portfolio.id}"
                )
...
def reactivate_domain_invitation(email: str, domain: Domain):
    ...
    try:
        with transaction.atomic():
            ...
            if reactivated:
                logger.info(
                    f"Reactivated domain invitation for {email} "
                    f"to domain {domain.id}"
                )
...
def check_duplicate_portfolio_invitation(email: str, portfolio: Portfolio):
    ...
    # Check new model
    if UserPortfolioPermission.objects.filter(
